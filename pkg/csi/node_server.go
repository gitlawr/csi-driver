...
func (ns *NodeServer) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	...
	logrus.Infof("getDevicePathByVolumeId: %v,%v", devicePath, err)
	if err != nil {
		return nil, status.Error(codes.Internal, fmt.Sprintf("failed to get device path for volume %v: %v", req.VolumeId, err))
	}
